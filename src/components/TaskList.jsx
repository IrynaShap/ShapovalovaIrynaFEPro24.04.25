export default function TaskList({ items, onView, onEdit, onDelete, onToggle }) {
  return (
        ...
        return (
          <li key={id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white/70 p-3 shadow-sm backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              />
              <span className={`truncate pr-3 font-medium text-slate-800 ${item.completed ? 'line-through text-slate-400' : ''}`}>{item.title}</span>
            </div>
